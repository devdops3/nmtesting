...
/// <summary>
/// Summary description for MoMoFinanceMananger
/// </summary>
public class MoMoFinanceMananger
{
	public MoMoFinanceMananger()
	{
		//
...
, string branchCode, string senderName)
    {
        Utils.WriteLog_Biller("This is MoMo finance from EBA Confirm");
        ...
        var confirmReq = new EbaConfirmReq()
        {
            ...
            BillerCode = ConfigurationManager.AppSettings["MoMoFinanceBillerCode"].ToString(),
            TransactionAmount = amount,
            Detail = "{'ContractID':'" + confirmResponseModel.ref1
        ...
        Utils.WriteLog_Biller("EBA MoMo finance Confirm Jason Request:" + jsonReq);

        SSLPost post = new SSLPost();
        string jsonres = Utils.PaymentRequest(jsonReq, BISConstants.EBAConfirmUrl);
        Utils.WriteLog_Biller("EBA MoMo finance Confirm Jason Response:" + jsonres);
        ...
        if (confirmRes.ErrorCode == "00")
        {
            ...
            if (appType == "CS" || appType == "MS")
            {
                ...
                if (string.IsNullOrEmpty(topupType) || topupType == "S") //topup type is null or S
                {
                    ...
                    smsMsg = smsH.getMessageBiller(agentName, MapTaxID, confirmResponseModel.billername, confirmResponseModel.ref1Name, confirmResponseModel.ref2Name, string.Empty, "Ref", confirmResponseModel.ref1,
                          confirmResponseModel.ref2, string.Empty, txnID.ToString(), double.Parse(amount).ToString("#,##0.00"), serviceFee, double.Parse(totalAmount).ToString("#,##0.00"), branchCode);
